...
//      B[k][j] = k;
...

  {
    ...
    cl::Kernel krnl(program, "network");

    // Allocate memory in host memory
    std::vector<unsigned short, aligned_allocator<unsigned short>> dev_A((64) * (64));
    std::vector<unsigned short, aligned_allocator<unsigned short>> dev_B((64) * (64));
    std::vector<unsigned short, aligned_allocator<unsigned short>> dev_C((64) * (64));

    // Initialize host buffers
    std::copy(reinterpret_cast<unsigned short *>(A), reinterpret_cast<unsigned short *>(A) + (64) * (64), dev_A.begin());
    std::copy(reinterpret_cast<unsigned short *>(B), reinterpret_cast<unsigned short *>(B) + (64) * (64), dev_B.begin());
    ...
    std::copy(dev_C.begin(), dev_C.end(), reinterpret_cast<unsigned short *>(C));
  }
